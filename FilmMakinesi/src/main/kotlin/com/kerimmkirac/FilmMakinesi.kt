...
class FilmMakinesi : MainAPI() {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        Log.d("kerimmkirac_$name", "arama = $document")
...
//        Log.d("kerimmkirac_$name", "match $match")

        val realUrl = if (dchelloVar.contains("var")) {
            val parts      = match?.groupValues[1].toString()
            Log.d("kerimmkirac_$name", "parts $parts")
            val decodedUrl = dcHello(parts)
            Log.d("kerimmkirac_$name", "decodedUrl $decodedUrl")
            decodedUrl
        } else{
            val parts = match!!.groupValues[1]
                .split(",")
                .map { it.trim().removeSurrounding("\"") }
            Log.d("kerimmkirac_$name", "dc parts: $parts")
            val decodedUrl = dcDecode(parts)
            Log.d("kerimmkirac_$name", "decoded URL: $decodedUrl")
            decodedUrl
        }
...
fun dcHello(encoded: String): String {
    ...
    Log.d("kerimmkirac_hdfilmcehennemi", "firstDecoded $firstDecoded")
    // Ters çevir
    val reversed = firstDecoded.reversed()
    Log.d("kerimmkirac_hdfilmcehennemi", "reversed $reversed")
    // İkinci Base64 çöz
    val secondDecoded = base64Decode(reversed)

    val gercekLink    = secondDecoded.substringAfter("http")
    val sonLink       = "http$gercekLink"
    Log.d("kerimmkirac_hdfilmcehennemi", "sonLink $sonLink")
    return sonLink.trim()
