...
class DiziBox : MainAPI() {
    ...
    private fun Element.toMainPageResult(): SearchResponse? {
        ...
        val posterUrl =
            fixUrlNull(this.selectFirst("img")?.attr("data-src")) ?: fixUrlNull(this.selectFirst("img")?.attr("src"))
        val href =
            fixUrlNull(this.selectFirst("h3 a")?.attr("href")) ?: (this.selectFirst("a.figure-link")?.attr("href")
        ...
        return newTvSeriesSearchResponse(title, href, TvType.TvSeries) {
            this.posterUrl = posterUrl; this.score = Score.from10(rating)
        }
    }
    ...
    private suspend fun iframeDecode(
        ...
        val subDoc = app.get(
            iframe.replace("php?v=", "php?wmode=opaque&v="),
            referer = data,
            cookies = mapOf(
                "LockUser" to "true",
                "isTrustedUser" to "true",
        ...
        val sheilaUrl = embedUrl.replace("/embed/", "/embed/sheila/").replace("vidmoly.me","vidmoly.net")

        Log.d("DZBX", "sheilaUrl url = $sheilaUrl")


        if (sheilaUrl.contains("dbx.molystream")){
            ...
            loadExtractor(sheilaUrl, subtitleCallback, callback)
        }
        return true
    }
