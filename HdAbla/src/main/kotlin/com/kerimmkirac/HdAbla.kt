...
class HdAbla : MainAPI() {
    ...
    if (iframeElement != null) {
        ...
        if (iframeSrc.isNotEmpty()) {
            ...
            Log.d("STF", "iframe url » $fullIframeUrl")
            
            val iframeDoc = app.get(fullIframeUrl, referer = mainUrl).document
            
            val scriptTags = iframeDoc.select("script")
            val foundUrls = mutableSetOf<String>() // Duplicate URL'leri engellemek için
            
            scriptTags.forEach { script ->
                val scriptContent = script.html()
                
                // Tüm pattern'leri tek seferde kontrol et
                val patterns = listOf(
                    """file\s*:\s*['"]([^'"]+)['"]""".toRegex(),
                    """["']([^"']*\.m3u8[^"']*)["']""".toRegex(),
                    """["']([^"']*\.mp4[^"']*)["']""".toRegex(),
                ...
                patterns.forEach { pattern ->
                    pattern.findAll(scriptContent).forEach { match ->
                        ...
                        // Sadece geçerli video URL'lerini kontrol et ve duplicate'leri engelle
                        if (videoUrl.startsWith("http") && 
                            ...
                            callback.invoke(
                                newExtractorLink(
                                    ...
                                    this.quality = if (videoUrl.contains("1080")) Qualities.P1080.value 
                                                 else if (videoUrl.contains("720")) Qualities.P720.value
                                                 else if (videoUrl.contains("480")) Qualities.P480.value
